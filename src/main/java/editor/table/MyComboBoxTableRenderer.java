 ...
 * @author spleaner
 */
public class MyComboBoxTableRenderer<T> extends JLabel implements TableCellRenderer, TableCellEditor, JBPopupListener {
    ...
    public MyComboBoxTableRenderer(final T[] values) {
        myValues = values;
        setFont(UIUtil.getButtonFont());
        setBorder(new EmptyBorder(0, 5, 0, 5));
    }

    public void updateValues(T[] myValues) {
    ...
    private void showPopup(final T value, final int row) {
        ...
        final ListPopup popup = JBPopupFactory.getInstance().createListPopup(new ListStep<T>(filtered, value) {
            ...
            public String getTextFor(T value) {
                return MyComboBoxTableRenderer.this.getTextFor(value);
            }

            @Override
            public Icon getIconFor(T value) {
                return MyComboBoxTableRenderer.this.getIconFor(value);
            }

            @Nullable
            @Override
            public ListSeparator getSeparatorAbove(T value) {
                return MyComboBoxTableRenderer.this.getSeparatorAbove(value);
            }

            public PopupStep onChosen(T selectedValue, boolean finalChoice) {
                myFinalRunnable = MyComboBoxTableRenderer.this.onChosen(selectedValue);
                return FINAL_CHOICE;
            }

            public void canceled() {
                MyComboBoxTableRenderer.this.cancelCellEditing();
            }
